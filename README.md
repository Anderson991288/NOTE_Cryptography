...
學習如何思考密碼構造的安全性，以及如何將這些知識應用於實際應用中。當一個強大的對手竊聽和干擾通訊時，兩個具有共享秘密密鑰的方可以安全通訊的方式。檢視許多已部署的協議並分析現有系統中的錯誤。再來討論了讓兩個方生成共享秘密密鑰的公鑰技術。涵蓋相關的數論並討論公鑰加密和基本金鑰交換。
...
一個區塊密碼的安全性可以透過其對應於偽隨機函數（PRF）或偽隨機置換（PRP）的性質來衡量。PRF和PRP都應該看起來像是隨機的，也就是說，對於一個沒有密鑰的攻擊者來說，他們不能區分加密後的結果和隨機數據的區別。
...
雖然DES在當時是革命性的，但由於其56位元的密鑰長度在現代被認為太短，容易受到窮舉攻擊（即嘗試每一個可能的密鑰）。這導致了後來AES（高級加密標準）的誕生，AES有更長的密鑰並且被認為更安全。
